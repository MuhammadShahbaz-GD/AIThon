...
namespace KickTheBuddy.Gameplay
{
    ...
    public sealed class LevelsManager : MonoBehaviour
    {
        ...
        public void Initialize(GameSaveManager saveManager)
        {
            saves = saveManager;
            int lastUnlocked = Mathf.Min(saves.Data.highestUnlockedLevel, Mathf.Max(0, Count - 1));
            CurrentLevelIndex = Mathf.Clamp(saves.Data.selectedLevel, 0, lastUnlocked);
        }
        ...
        public bool SelectLevel(int index)
        {
            ...
            saves.RecordLevelSelection(index, false);
            LevelSelected?.Invoke(index, CurrentLevel);
            return true;
        }
