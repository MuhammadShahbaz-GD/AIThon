...
{
    /// <summary>
    /// Small public facade for the ragdoll subsystem. Calculations are owned by focused sibling components.
    /// </summary>
    ...
    public sealed class RagdollController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            rig = GetOrAdd(rig);
            pose = GetOrAdd(pose);
            profiles = GetOrAdd(profiles);
            state = GetOrAdd(state);
            damageManager = GetOrAdd(damageManager);
            ...
            damageManager.RefreshParts();

            if (GetComponent<RagdollElementalEffects>() == null)
                gameObject.AddComponent<RagdollElementalEffects>();
            if (GetComponent<RagdollAnimationController>() == null)
                gameObject.AddComponent<RagdollAnimationController>();
        }
        ...
        {
            profiles?.ApplyProfile(newProfile);
        }

        public void SelectCategory(RagdollCategory category)
        {
            profiles?.SelectCategory(category);
        }
        ...
        {
            state?.SetLimpState(makeLimp);
        }

        public void Knockout(float duration)
        {
            state?.Knockout(duration);
        }
        ...
        internal void SynchronizeAggregateHealth(
            float combinedHealth,
            float combinedMaximumHealth,
            float appliedDamage,
            float impactSpeed,
            Vector2 point)
        {
            ...
            if (appliedDamage > 0f)
            {
                ...
                if (profile != null)
                    OnProfileDamageEffect?.Invoke(profile.ProfileType, profile.DamageParticleColor, point);
                OnImpactResolved?.Invoke(appliedDamage, impactSpeed, point);
            }

            if (currentHealth <= 0f && CurrentState != RagdollState.KnockedOut)
                state?.KnockoutDefault();
        }
        ...
        internal void NotifyProfileApplied(RagdollProfile profile)
        {
            OnProfileApplied?.Invoke(profile);
        }

        internal void NotifyLimpStateChanged(bool limp)
        {
            OnLimpStateChanged?.Invoke(limp);
        }

        internal void NotifyKnockoutStarted(float duration)
        {
            OnKnockoutStarted?.Invoke(duration);
        }

        internal void NotifyCharacterKnockedOut()
        {
