...
namespace KickTheBuddy.Editor
{
    public static class GameSetupEditor
    {
        ...
        private static void CreateHUD()
        {
            ...
            Text level = Label(panel.transform, "Level", new Vector2(0, -55), 38, TextAnchor.UpperCenter); Text objective = Label(panel.transform, "Objective", new Vector2(0, -110), 26, TextAnchor.UpperCenter); Text score = Label(panel.transform, "Score", new Vector2(-360, -55), 34, TextAnchor.UpperLeft); Text timer = Label(panel.transform, "Timer", new Vector2(360, -55), 34, TextAnchor.UpperRight);
            SerializedObject hud = new SerializedObject(canvasObject.GetComponent<GameplayHUD>()); hud.FindProperty("levelText").objectReferenceValue = level; hud.FindProperty("objectiveText").objectReferenceValue = objective; hud.FindProperty("scoreText").objectReferenceValue = score; hud.FindProperty("timerText").objectReferenceValue = timer; hud.FindProperty("gameplayPanel").objectReferenceValue = panel; hud.ApplyModifiedPropertiesWithoutUndo();
        }
